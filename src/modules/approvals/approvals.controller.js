...
exports.deleteItems = async (req, res) => {
  try {
    ...
    const items = Array.isArray(body.items) ? body.items : [];
    const actorId = getActorId(req);
    const result = await svc.deleteItems(items, actorId);
    res.json(result);
  } catch (err) {
    console.error("approvals.deleteItems error:", err);
    res.status(err.status || 500).json({ message: err.message || "Silme hatası" });
  }
};
